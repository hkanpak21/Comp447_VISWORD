...
Hyperparameters in Table~\ref{tab:hparams}.
Heads are either an MLP ($768{\to}512{\to}256$) or the SALAD
optimal-transport aggregator \cite{izquierdo2024salad} (8\,448-d).
The CLIP/SigLIP backbone LR of $10^{-7}$ is the highest that
avoided immediate representation collapse; DINOv2 trains stably
at $10^{-5}$.

\begin{table}[t]
\caption{Training hyperparameters. Track A is the headline 30\,k
grid, Track B the 50\,k saturation diagnostic.}
\label{tab:hparams}
\centering
\small
\begin{tabular}{ll}
...
field & value \\
\midrule
batch & 4 pages $\times$ 4 crops $= 16$ \\
loss & multi-similarity \cite{wang2019msloss} \\
optimiser & AdamW, wd $=10^{-4}$, clip $=1.0$ \\
schedule & 5\,\% linear warmup $\to$ cosine to $0$ \\
$\eta_{\text{backbone}}$ & $10^{-5}$ (DINOv2), $10^{-7}$ (CLIP/SigLIP) \\
$\eta_{\text{head}}$ & $5{\times}10^{-4}$ \\
trainable & last 4 of 12 ViT blocks $+$ head \\
epochs & 3 \\
train pages & 30\,000 (Track A), 50\,000 (Track B) \\
\bottomrule
\end{tabular}
\end{table}
...
triplets from an in-house anchor-pool slice we derive from
\texttt{Tevatron/wiki-ss-corpus}: each triplet pairs an anchor crop
with positive same-page crops sampled from outside the anchor's
non-overlapping crop neighbourhood plus hard-negative crops drawn
from unrelated pages.\footnote{Anchor-pool slice (118 valid validation
triplets) constructed once from the same corpus as Protocol-A; not
re-used for training.} For each anchor
...
level: a post-hoc text adapter (\S\ref{sec:adapter}) is the only
...
0.932 is tie-breaking in a 7-element pool, not retrieval.}
\end{tabular}
\end{table}
...
its R@10 by $0.241$ but rank-1 LEACE leaves it untouched
($0.911 \to 0.912$). A rank sweep recovers the blanking delta
near rank-64: $\Delta_{\text{LEACE}} = -0.030, -0.136, -0.266$
at ranks $4, 16, 64$. Pretraining gives the image-text encoders
a single-axis title confound; fine-tuning on Wikipedia spreads
the analogous binding across a $\approx 16$--$64$-d subspace of
the $8\,448$-d SALAD descriptor.

\section{Discussion and Limitations}
\label{sec:disc}

Single-seed; cross-family ordering reads past noise
($\Delta$R@10 $\ge 0.09$), within-family numbers need multi-seed
reruns. $P{=}2000$ is small versus industrial corpora; numbers
are within-table comparable. At $224^2$ input body text is
sub-pixel, so ``reading'' here means heading-level text plus
layout. English Wikipedia only. All experiments run on one Tesla
T4, ${\approx}66$ GPU-h total; code at
\url{github.com/hkanpak21/Comp447_VISWORD}, eval seed $42$.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.6\linewidth]{figures/blank_example.pdf}
\caption{Pixel-level title blanking used in \S\ref{sec:blank}: the
top 15\,\% of every eval page is painted white before cropping.
``Reading'' the title text is no longer possible after blanking;
the headers, infoboxes, and body remain.}
\label{fig:blank-example}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.7\linewidth]{figures/leace_rank_sweep.pdf}
\caption{LEACE rank sweep on trained DINOv2-SALAD-50k
(\S\ref{sec:leace}). The Protocol-A R@10 of the LEACE-erased
descriptors is plotted against the rank of the protected
attribute $Z$. The pixel-blanking delta ($-0.241$, dashed orange)
is matched at rank $\approx 64$, indicating the trained
title-region binding lives in a $16$--$64$-dim subspace of the
$8\,448$-d SALAD descriptor.}
\label{fig:leace-sweep}
\end{figure}
