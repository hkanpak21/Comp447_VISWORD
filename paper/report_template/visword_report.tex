...
Pixel-blanking is correlational: it removes title content but also
whatever else lives in those pixels. We isolate the
\emph{title-direction} in feature space using LEACE
\cite{belrose2023leace} with the rank-1 PCA projection of the
per-crop deltas $\phi(\mathrm{orig})-\phi(\mathrm{blank})$ as the
protected attribute (higher-rank $Z$ over-erases, since
$\mathrm{rank}(Z){=}d$ collapses the entire feature space).
The result on $P{=}2000$ zero-shot:
CLIP R@10 $0.779 \to 0.823$ (blank) $\to 0.863$ (LEACE);
DINOv2 R@10 $0.058 \to 0.056 \to 0.068$. LEACE-erasing a single
axis raises CLIP retrieval more than physical blanking ($+0.084$
vs $+0.044$): the title-direction's dominant linear axis carries
enough of CLIP's confounding signal that surgical removal beats
the pixel intervention. DINOv2 is unmoved by either --- the title
direction barely exists as a structured axis in its feature space.
