...
retrieval untouched (0.912). The trained model's title-region
dependency is not summarised by a single linear axis. The 8\,448-d
SALAD descriptor distributes the binding across many directions,
and a higher-rank LEACE would be required to recover the blanking
delta. This is consistent with the
\S\ref{sec:blank} reading: pretraining gives CLIP a clean, low-rank
title confound; supervised fine-tuning on Wikipedia spreads an
analogous title-region binding across DINOv2's feature space.
