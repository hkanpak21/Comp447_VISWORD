...
# ---------------------------------------------------------------------------
# Tensor operations (from Meta's src/utils/tensors.py and src/masks/utils.py)
# ---------------------------------------------------------------------------
...
def apply_masks(x: torch.Tensor, masks: list[torch.Tensor] | torch.Tensor) -> torch.Tensor:
    """Apply masking by gathering the token indexes to keep.

    :param x: tensor of shape [B, N, D]
    :param masks: list of tensors (length nenc) or a single tensor of shape [B, min_keep]
    """
...
def repeat_interleave_batch(x: torch.Tensor, B: int, repeat: int) -> torch.Tensor:
    """Repeats and interleaves batch blocks."""
    N = len(x) // B
    x = torch.cat([
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        if return_attention:
            return self.attn(self.norm1(x))[1]
